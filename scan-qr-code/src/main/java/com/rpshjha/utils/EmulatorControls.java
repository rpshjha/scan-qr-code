...
public class EmulatorControls {
    ...
    public static void editEmulatorToren1BDFile() {
        String[] command = new String[]{"sh", "bashscript/edit_emulator_file.sh"};
        String line;
        try {
            Process process = new ProcessBuilder(command).start();
            BufferedReader input = new BufferedReader(new InputStreamReader(process.getInputStream()));
    ...
    public static void killActiveEmulators() {
        String[] getDevices = new String[]{ADB_PATH, "devices"};

        String line;
        List<String> runningEmulator = new ArrayList<>();
        Process process;
        try {

            process = new ProcessBuilder(new String[]{"sh", "bashscript/kill_all_avd.sh"}).start();
            process.waitFor(10, TimeUnit.SECONDS);

//            System.out.println("executing command " + String.join(" ", getDevices));
//            process = new ProcessBuilder(getDevices).start();
//            BufferedReader input = new BufferedReader(new InputStreamReader(process.getInputStream()));
//            while ((line = input.readLine()) != null) {
//                String temp = line.split("\\s")[0].trim();
//                if (!line.contains("List of devices attached") && !temp.isEmpty())
//                    runningEmulator.add(temp);
//            }
//
//            LOGGER.info("Running emulators are " + runningEmulator);
//            for (String temp : runningEmulator) {
//                System.out.println("executing command " + String.join(" ", new String[]{ADB_PATH, "-s", temp, "emu kill"}));
//                process = new ProcessBuilder(new String[]{ADB_PATH, "-s", temp, "emu kill"}).start();
